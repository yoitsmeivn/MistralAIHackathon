...
async def text_to_speech(
    ...
    # SDK: text_to_speech.convert() returns Iterator[bytes]
...
async def speech_to_text(
    ...
    """
    LOGGER.debug(
        "ElevenLabs STT: filename=%s bytes=%d", filename, len(audio_bytes)
    )
...
async def speech_to_text_from_url(
    ...
    """
    Transcribe audio from a public HTTPS URL using ElevenLabs STT.

    Passes cloud_storage_url so ElevenLabs fetches the file server-side.
    Useful for Twilio RecordingUrl — no need to download the audio first.
    ...
    transcript = result.text or ""
    LOGGER.debug(
        "ElevenLabs STT from URL: lang=%s text_len=%d",
        getattr(result, "language_code", "?"),
        len(transcript),
    )
    return transcript
...
async def realtime_stt_session(
    ...
    params = [
        ...
        "include_timestamps=false", # we only need text, not word timing
        ...
        async def _receive_transcripts() -> AsyncIterator[str]:
            ...
            async for raw in ws:
                ...
                if msg_type == "session_started":
                    ...
                    LOGGER.debug(
                        "ElevenLabs Realtime STT partial: %r", msg.get("text")
                    )
                    ...
                    if text:
                        LOGGER.info(
                            "ElevenLabs Realtime STT committed: %r", text
                        )
                        yield text
